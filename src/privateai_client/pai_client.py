...
class PAIClient:
    ...
    def analyze_text(self, request_object: Union[dict, AnalyzeTextRequest]):
        """
        Used to analyze text
        """
        if type(request_object) is AnalyzeTextRequest:
            self.check_version_compatibility()
            response = AnalyzeTextResponse(self.post.analyze_text(request_object.to_dict()))
            request_object = request_object.to_dict()
        elif type(request_object) is dict:
            self.check_version_compatibility()
            response = AnalyzeTextResponse(self.post.analyze_text(request_object))
        else:
            raise ValueError("request_object can only be a dictionary or a AnalyzeTextRequest object")
        return response
