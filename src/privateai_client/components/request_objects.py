...
def ProcessText(
    type: str = "MARKER",
    pattern: str = "UNIQUE_NUMBERED_ENTITY_TYPE",
    values: Dict = None,
):
    ...
    if values != None:
        if "type" in values.keys():
            if values["type"] in valid_types:
                ...
                raise ValueError(
                    f"{values['type']} is not valid. ProcessedText.type can only be one of the following: {', '.join(valid_types)}"
                )
        else:
        ...
        if "pattern" in values.keys():
            if values["pattern"] in valid_patterns:
                ...
                raise ValueError(
                    f"{values['pattern']} is not valid. ProcessedText.pattern can only be one of the following: {', '.join(valid_patterns)}"
                )
        else:
    ...
    if type == "MARKER":
        ...
        raise ValueError(
            f"{type} is not valid. ProcessedText.type can only be one of the following: {', '.join(valid_types)}"
        )
...
class ProcessTextRequest(BaseRequestObject):
    ...
    def __init__(
        ...
        processed_text: Union[
            ProcessedMarkerText, ProcessedMaskText, ProcessedSyntheticText
        ] = ProcessText(),
    ):
        self.text = text
